  ...
  def step(self):
    ...
    for group in self.param_groups:
      ...
      for base_i in range(len(params))[:: self.world_size]:
        ...
        futures.append(
          dist.all_gather(
            params_pad[base_i : base_i + self.world_size], params_pad[base_i + self.rank], async_op=True
          ).get_future()
        )
...
class CausalSelfAttention(nn.Module):
  ...
  def forward(self, x: Tensor, ve: Tensor | None, block_mask: BlockMask, lambdas: Tensor):
    ...
    q, k, v = (
      F.linear(x, self.qkvo_w[:3].flatten(end_dim=1)).view(B, T, 3 * self.num_heads, self.head_dim).chunk(3, dim=-2)
    )
...
class Block(nn.Module):
  ...
  def forward(
    self, x: Tensor, ve: Tensor | None, x0: Tensor, block_mask: BlockMask, lambdas: Tensor, sa_lambdas: Tensor
  ):
...
hidden_matrix_params = sorted(
  (p for p in model.blocks.parameters() if p.ndim >= 2), key=lambda x: x.size(), reverse=True
)
